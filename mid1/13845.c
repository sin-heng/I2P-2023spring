...
// 2023.4.5 second version -- pass 6/8
#include<stdio.h>
#include<stdlib.h>
...
// variable
char str[26][201]; // expression
long long num[5];
Node* func[26];
int nfunc,pos;
int main(void){

    int n,q; // n functions, q queries
    char c;
    scanf("%d %d",&n,&q);
    // build function tree
    for(int i=0;i<n;i++){
...
long long int cal(Node* root){
    ...
    if(c == '@'){
        if(65<=right&&right<=90){
            ...
            num[0] = num[right-64];
            ans = Mod(cal(func[left-'a']));
            num[0] = x;
            return ans;
        }
        else if(right=='+'||right=='-'||right=='*'||right=='@'){
            long long x = num[0],ans;
            num[0] = Mod(cal(root->right)); // different from above
            ans = Mod(cal(func[left-'a']));
            num[0] = x;
            return ans;
        }
    }
    else if(c=='+') return Mod(cal(root->left) + cal(root->right));
    else if(c=='-') return Mod(cal(root->left) - cal(root->right));
    else if(c=='*') return Mod(cal(root->left) * cal(root->right));
}
Node* build(){
